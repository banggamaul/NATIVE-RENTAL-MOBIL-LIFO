...
<body>
    ...
    include("konek.php");
    $id = $_GET['id'];
    $caari = mysqli_query($konek, "SELECT * FROM mobil where id_mobil='$id'");
    ...
    $daata = mysqli_fetch_array($caari);

    ?>
    <div class="card border-secondary mt-4 mx-auto" style="max-width: 50rem;">
        <div class="card-header">Form Order</div>
        <div class="card-body text-secondary">
            <form method="post" action="?x=sorderpelanggan">
                <input type="hidden" name="kode" value="<?php echo $daata['id_mobil']; ?>">
                <div class="mb-3">
                    <label class="form-label">Mobil</label>
                    <select name="txtnopol" class="form-control">
                        <option value="<?php echo $daata['id_mobil']; ?>">
                            <?php echo $daata['no_polisi'] . " -" . $daata['merk'] . " Rp." . $daata['harga']; ?></option>
                        <?php
                ...
                <input type="hidden" name="txtsmobil" value="Di sewa">
